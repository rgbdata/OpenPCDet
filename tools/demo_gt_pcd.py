...
from pcd_utils import point_cloud_from_fileobj
...
def draw_points_with_boxes(points, gt_boxes=None):
    """
...
def parse_args():
    ...
    parser.add_argument('--pcd_path', type=str, default='data/caic_bev/perception/1717586343.885023117.pcd')
...
def main():
    ...
    tmp = pcd_path.split('/')
    basename = tmp[-1].split('.pcd')[0]
    gt_path = os.path.join('/'.join(tmp[:-2]), 'label_biaozhu', basename+'.json')
