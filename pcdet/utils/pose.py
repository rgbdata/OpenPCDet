...
class Pose:
    ...
    @property
    def yaw(self):
        return rot2euler(self.rotation_matrix)[-1]
    ...
    def from_matrix(cls, transformation_matrix):
        ...
        Pose
        """
        return cls(wxyz=Quaternion(matrix=transformation_matrix[:3, :3], atol=1e-6, rtol=1e-6), tvec=np.float32(transformation_matrix[:3, 3]))

    @classmethod
    def from_rotation_translation(cls, rotation_matrix, tvec, atol=1e-6, rtol=1e-6):
        ...
        """
        return cls(wxyz=Quaternion(matrix=rotation_matrix, rtol=rtol, atol=atol), tvec=np.float64(tvec))

    @classmethod
    def from_box(cls, xyz, yaw):
        return cls(
                    wxyz=Quaternion(axis=[0, 0, 1], radians=yaw),
                    tvec=np.float64(xyz))

    @classmethod
    def from_yaw(cls, yaw):
        return cls(wxyz=Quaternion(axis=[0, 0, 1], radians=yaw), tvec=np.array([0, 0, 0]))

    def transform(self, box):
        box_copy = box.copy()
        tmp = self * Pose.from_box(box_copy[:3], box_copy[6])
        box_copy[:3] = tmp.tvec
        box_copy[6] = tmp.yaw
        return box_copy
    ...
    def transform_points(self, points):
        ...
        return (self.matrix @ points_homo.T).T[:, :3]
